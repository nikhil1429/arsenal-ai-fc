...
- **Captain's-shorthand = the build-ceiling metaphor (thread-agnostic, 11 Jul).** When the captain says "god-tier," "10x," "on steroids," "beast-mode" (or the like) about a **build**, it is a **metaphor for the elevated ceiling** — the deepest / highest **depth-of-EFFECT per load-bearing word** achievable (ruthless, non-duplicative, scope-matched; OPS_STATE 🔒 "ELEVATED CEILING"). It is **never** a request for hype, and it never licences hype in any agent's **output** — "never sell exponential" stands unchanged on every sheet. A thread **reaches for that ceiling**: it does not hedge/soften the phrase as if hype were being asked for, and it does not inject hype to satisfy it. **Build = god-tier metaphor · output = honest compounding** — two registers, never conflated.
