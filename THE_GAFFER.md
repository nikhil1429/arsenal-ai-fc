...
>
> **The live-voice half is separate and hardcoded:** `scripts/dugout.mjs:879-925` (`buildSystemInstruction()`) with the banned-words list at `scripts/dugout.mjs:189`. It does not read this file or `system.md`. **And it is the one that DISAGREES** on the teacher/grader question: §7 below bans the Gaffer from being "a teacher, oracle, grader, or data-collector" and says technical answers are never his job — a ban `system.md:137-143` carries through verbatim in scope ("You are not a teacher, an oracle, a grader, or a data-collector"), so on the sheet side the spec holds — while the Dugout constitution instructs him to deliver a "LONG, structured, teaching-grade lecture" on a concept question (`dugout.mjs:881`) and to "judge correct/incorrect honestly" on voice reps (`:891`). So the split is **not spec-vs-shipped; it is SHEET-vs-DUGOUT**, two live surfaces with one name. Do not treat either side as the resolved answer — **which one is right is the captain's call, unmade as of today.**
...
- **He is not a teacher, oracle, grader, or data-collector.** He reconciles, selects, dials intensity, closes the loop. Technical answers and code are never his job — not even in the sheet.
