...
#
# ============================ ASCII ONLY. THIS IS A BUG SCAR. ================
# The first version of this file was written in UTF-8 WITHOUT a BOM and carried
# em-dashes and arrows in its comments. Windows PowerShell 5.1 - which is what
# `powershell` resolves to on this box, not PowerShell 7 - reads a .ps1 as ANSI
# unless it finds a BOM. Every multi-byte character therefore came apart, the
# byte drift terminated a string early, and the FIRST thing he ever saw was
#     y/N : The term 'y/N' is not recognized as the name of a cmdlet
# from `Read-Host "  Replace it? (y/N)"` on line 44. Nothing on that line was
# wrong. A static parse check had passed it, which is exactly why it shipped: the
# parser was handed the file with the right encoding and the shell was not.
# SO: no character in this file may be outside 7-bit ASCII, and the selftest
# asserts it (grep -n "ASCII ONLY" scripts/gaffer_brain.mjs). A BOM would also fix
# it, but staying ASCII fixes it everywhere and can be checked by a machine.
# =============================================================================
#
...
# -AsSecureString: the key never echoes to the screen and never enters
# PSReadLine's history file. It is converted in-memory only, then zeroed.
