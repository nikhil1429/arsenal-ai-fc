...
| **2 · Sitting** | Sit. **Voice:** the DUGOUT icon (`setup/launchers/`) — **the sitting opens ITSELF** the moment the Gaffer's page loads (Block 5, his 18 Aug 10:42 word "bro i will never remember any of the keyword"; `node scripts/sitting.mjs status` shows it open) — Re-Jirah recital + bank/judge with the Gaffer. **Text:** `/learn` in Claude Code (reads the state, routes: concept → `/forge`, due Re-Jirah → round, staged mock → scrimmage) — it registers a code sitting on the SAME mind. Say your gut-word before every answer. | ONE mind behind every mouth (`scripts/sitting.mjs`, :4117 — L2): paces THE METHOD (`node scripts/forge_session.mjs contract`), banks every answer instantly, judges the round with ONE Opus call, captures the reps — no copy, no paste. |
| **3 · Full-time** | Say *"full time"* to the Gaffer, or `/full-time` in Claude Code, or the FULL TIME icon — HIT/MISS · one signal · KAL-line. Then sleep. A full-time said after midnight still closes THE DAY IT BELONGS TO (the day-key law, Block 6 — `node scripts/herd.mjs risks` — it prints how many organs are still guessing the day (read it; a number frozen here is a lie by tomorrow) from the clock). | Scores the day, stages tomorrow, runs the dark lane behind THE GATE (`node scripts/brain.mjs gate show`). |
...
- **The sitting brain — BUILT 18 Aug 2026 (overhaul Block 3), auto-opening since Block 5:** one persistent Claude session behind every mouth (`sitting.mjs`, port 4117, `node scripts/sitting.mjs status` · `stats` for the week). If :4117 is down, the watchdog boots it — `schtasks /Run /TN "ArsenalFC-Daemon-Watchdog"`, never `node scripts/sitting.mjs daemon` from a session shell (a daemon born in a session dies with it — measured 18 Aug 11:10). The overhaul's record: `docs/archive/ORGANISM_OVERHAUL__2026-08-18.md` (BUILD LOG at its top); the freeze: DEFERRED by his word 18 Aug 2026 evening (`node scripts/freeze.mjs status`; record `docs/archive/FREEZE__deferred-2026-08-18.md`).
